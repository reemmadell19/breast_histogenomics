...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, alpha=[0.25, 0.75]):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha  # Keep as list for now
...
ATTENTION_HIDDEN_DIM = 128  # Attention hidden dimension
...
LABEL_SMOOTHING = 0.2        # Label smoothing for criterion (confidence in predictions)
ALPHA = [0.25,0.75]
...
def plot_validation_roc_curves(all_histories_df, save_dir, model_name, mil_architecture):
    """
    Plot validation ROC curves across all CV folds with mean and std
    
    This shows how validation performance varies across folds during training
    """
    # Get the best epoch for each fold (where val_auroc was highest)
    best_epochs = []
    ...
    # For each fold, get the validation AUROC at each epoch
    folds = all_histories_df['fold'].unique()
    epochs = sorted(all_histories_df['epoch'].unique())
    
    # Store AUROC values for calculating mean/std
    auroc_by_epoch = {epoch: [] for epoch in epochs}
    
    # Plot individual fold validation AUROC curves
    for fold in folds:
        fold_data = all_histories_df[all_histories_df['fold'] == fold]
        fold_aurocs = []
        fold_epochs = []
...
def run_cv_training(model_name, mil_architecture):
    ...
    # ADD THIS LINE - Plot validation ROC curves across folds
