# File: train_baseline_bbb.py
# Baseline Model Training with Batch Balancing (BBB) - Cleaned Version
...
# Configuration
CONFIG = {
    "model_name": "baseline",  # Generic baseline model (likely ResNet18)
    "experiment_type": "bbb",
    "input_dim": 512,  # Standard ResNet18 feature dimension
    "batch_size": 1,
    "num_classes": 2,
    "lr": 1e-4,
    "num_epochs": 10,
...
def main():
    ...
    wandb_config = {
        ...
        "backbone": "Enhanced-BBB",  # Keep original naming for continuity
        "loss": "CrossEntropy",
        "sampling": "WeightedRandomSampler",
        **{k: v for k, v in CONFIG.items() if k not in ["train_csv", "val_csv"]}
    }
    initialize_wandb("rs-baseline-mil", "enhanced-mean-pooling-bbb-baseline", wandb_config)
