# File: train_weighted_loss_enhanced.py (enhanced weighted loss training)
...
results_dir = "results/weighted_loss"
os.makedirs(results_dir, exist_ok=True)
print(f"📁 Results will be saved to: {results_dir}")

# Class weights calculation
class_frequencies = {"Class 0 (Low Risk)": 0.811, "Class 1 (High Risk)": 0.189}
# Inverse frequency weighting - higher weight for minority class
raw_weights = [1.0/0.811, 1.0/0.189]
...
print(f"⚖️  Raw inverse weights: {raw_weights}")
print(f"🎯 Normalized class weights: {class_weights.cpu().tolist()}")

# Initialize Weights & Biases
wandb.init(project="rs-baseline-mil", name="enhanced-weighted-loss-baseline")
wandb.config.update({
    "model": "MeanPoolingMIL",
    "loss": "CrossEntropy_Weighted",
...
for epoch in range(1, num_epochs + 1):
    ...
    wandb_log = {
        ...
        "val_mcc": val_metrics['mcc'],
        "val_balanced_accuracy": val_metrics['balanced_accuracy'],
        # Weighted loss specific
        "class_weight_0": class_weights[0].item(),
        "class_weight_1": class_weights[1].item(),
    ...
    if val_metrics['auc_roc'] > best_val_auc:
        ...
        save_path = os.path.join(results_dir, "best_weighted_loss_model.pt")
        ...
        val_evaluator.plot_confusion_matrix(
            ...
            title=f"Best Weighted Loss Model Confusion Matrix (AUC: {best_val_auc:.4f})"
        )

# ── Post-Training Analysis & Visualization ─────────────────────────────────
print("\n" + "="*60)
print("WEIGHTED LOSS TRAINING COMPLETE - GENERATING REPORTS")
print("="*60)
...
try:
    ...
    fig.suptitle("Weighted Loss Training Progress - Comprehensive Metrics", fontsize=16)
    ...
    axes[0,0].set_title('Loss Curves (Weighted CrossEntropy)')
    ...
    try:
        ...
        axes[0].set_title('Weighted Loss Curves')
...
print(f"\n📋 FINAL WEIGHTED LOSS PERFORMANCE SUMMARY:")
...
print(f"   - best_weighted_loss_model.pt")

# Also create a summary report
summary_report = f"""
WEIGHTED LOSS EXPERIMENT SUMMARY
{'='*50}
Model: MeanPoolingMIL with Weighted CrossEntropy Loss
...
WEIGHTED LOSS DETAILS:
- Class 0 (Low Risk):  Frequency={class_frequencies['Class 0 (Low Risk)']} → Weight={class_weights[0]:.4f}
- Class 1 (High Risk): Frequency={class_frequencies['Class 1 (High Risk)']} → Weight={class_weights[1]:.4f}
- Strategy: Inverse frequency weighting to balance class contributions
- Normalization: Weights normalized to sum to {num_classes}
...
WEIGHTED LOSS THEORY:
Weighted CrossEntropy modifies standard CrossEntropy by:
1. Assigning different importance to each class during loss computation
2. Higher weights for minority classes (high-risk patients in this case)
3. Helps model pay more attention to underrepresented classes
4. Can improve recall/sensitivity for minority class without data resampling

COMPARISON WITH OTHER APPROACHES:
- vs Baseline: Addresses class imbalance at loss level
- vs BBB: Addresses imbalance via loss weighting instead of sampling
- vs Focal Loss: Simpler approach focused on class frequency rather than example difficulty

FILES SAVED:
- Model: best_weighted_loss_model.pt
...
print(f"\n🎯 Weighted Loss Summary report saved to: {summary_path}")
