# File: train_weighted_focal_loss_enhanced.py (enhanced weighted focal loss training)
# Enhanced training script with comprehensive MIL histopathology metrics
...
# Weighted Focal Loss hyperparameters
class_frequencies = {"Class 0 (Low Risk)": 0.811, "Class 1 (High Risk)": 0.189}
class_weights = [1.0/0.811, 1.0/0.189]  # Inverse frequency weighting
focal_gamma = 2.0  # Focusing parameter (higher = more focus on hard examples)

# Create results directory (will overwrite previous runs)
results_dir = "results/weighted_focal_loss"
...
wandb.init(project="rs-baseline-mil", name="enhanced-weighted-focal-loss-baseline")
wandb.config.update({
...
print(f"🎯 Using Weighted Focal Loss with class weights={class_weights}, gamma={focal_gamma}")
...
for epoch in range(1, num_epochs + 1):
    ...
    if val_metrics['auc_roc'] > best_val_auc:
        ...
        save_path = os.path.join(results_dir, "best_weighted_focal_loss_model.pt")
        ...
        val_evaluator.plot_confusion_matrix(
            ...
            title=f"Best Weighted Focal Loss Model Confusion Matrix (AUC: {best_val_auc:.4f})"
        )

# ── Post-Training Analysis & Visualization ─────────────────────────────────
print("\n" + "="*60)
print("WEIGHTED FOCAL LOSS TRAINING COMPLETE - GENERATING REPORTS")
print("="*60)
...
try:
    ...
    fig.suptitle("Weighted Focal Loss Training Progress - Comprehensive Metrics", fontsize=16)
    ...
    axes[0,0].set_title('Loss Curves (Weighted Focal Loss)')
    ...
    try:
        ...
        axes[0].set_title('Weighted Focal Loss Curves')
...
print(f"\n📋 FINAL WEIGHTED FOCAL LOSS PERFORMANCE SUMMARY:")
...
print(f"   - best_weighted_focal_loss_model.pt")

# Also create a summary report
summary_report = f"""
WEIGHTED FOCAL LOSS EXPERIMENT SUMMARY
{'='*50}
Model: MeanPoolingMIL with Weighted Focal Loss
Loss Function: Weighted Focal Loss (combines class weighting + hard example focusing)
Class Frequencies: {class_frequencies}
...
- Class 0 (Low Risk):  Weight={class_weights[0]:.4f} (freq={class_frequencies['Class 0 (Low Risk)']})
- Class 1 (High Risk): Weight={class_weights[1]:.4f} (freq={class_frequencies['Class 1 (High Risk)']})
- Gamma: {focal_gamma} (focusing parameter - higher values focus more on hard examples)
...
WEIGHTED FOCAL LOSS THEORY:
Weighted Focal Loss combines two powerful techniques:

1. CLASS WEIGHTING (Alpha):
   - Addresses class imbalance by giving different importance to each class
   - Higher weights for minority classes (high-risk patients)
   - Similar to weighted CrossEntropy but integrated with focal mechanism

2. HARD EXAMPLE FOCUSING (Gamma):
   - Down-weights easy examples (high confidence predictions)
   - Focuses learning on hard examples (low confidence predictions)  
   - Gamma controls focusing strength (higher = more focus on hard cases)

3. SYNERGISTIC EFFECT:
   - Combines class balancing with difficulty-based learning
   - Should excel at detecting hard-to-classify minority class examples
   - Particularly beneficial for medical applications with rare positive cases

COMPARISON WITH OTHER APPROACHES:
- vs Baseline: Addresses both class imbalance AND hard example learning
- vs Weighted Loss: Adds hard example focusing to class weighting
- vs Focal Loss: Adds explicit class weighting to hard example focusing
- vs BBB: Loss-based approach instead of sampling-based

EXPECTED BENEFITS:
- Best sensitivity for high-risk cases (combines class weighting + hard example focus)
- Improved learning on challenging examples within minority class
- More robust to both class imbalance and example difficulty
- Better calibrated confidence scores

FILES SAVED:
- Model: best_weighted_focal_loss_model.pt
...
print(f"\n🎯 Weighted Focal Loss Summary report saved to: {summary_path}")
print(f"\nFinal class weights used: {class_weights}")
