...
HIDDEN_DIM = 16    # Hidden dimension for MIL classifier

# Attention-based models (attention, clam, acmil_clam)
ATTENTION_HIDDEN_DIM = 34  # Attention hidden dimension

# CLAM-specific
GATE = True           # Use gated attention in CLAM

# ACMIL-specific
N_BRANCHES = 2        # Number of branches in ACMIL
...
def plot_training_curves(fold_history_df, save_dir, fold_num, best_epoch=None):
   ...
   # 4. Balanced Accuracy curves 
    ...
    if 'train_balanced_accuracy' in fold_history_df.columns:
        ax.plot(epochs, fold_history_df['train_balanced_accuracy'], label='Train Balanced Acc', marker='o', markersize=4, linewidth=2)
    ax.plot(epochs, fold_history_df['val_balanced_accuracy'], label='Val Balanced Acc', marker='s', markersize=4, linewidth=2)
    if best_epoch:
        ax.axvline(x=best_epoch, color='red', linestyle='--', alpha=0.7, label=f'Best Epoch ({best_epoch})')
    ax.set_xlabel('Epoch', fontsize=12)
...
def plot_combined_cv_curves(all_histories_df, save_dir, model_name, mil_architecture, fold_best_epochs=None):
    ...
    # Helper function to add best epoch line
    def add_best_epoch_line(ax):
        if mean_best_epoch:
            ax.axvline(x=mean_best_epoch, color='red', linestyle='--', alpha=0.5, 
                      label=f'Avg Best Epoch ({mean_best_epoch:.1f})')
        if fold_best_epochs:
            # Also add individual fold best epochs as thin vertical lines
            for fold, best_ep in fold_best_epochs.items():
                ax.axvline(x=best_ep, color='pink', linestyle=':', alpha=0.3, linewidth=0.5)
    ...
    ax.set_xlabel('Epoch', fontsize=12)
    ax.set_ylabel('Balanced Accuracy', fontsize=12)
    ax.set_title('Balanced Accuracy over Epochs (Mean ± Std)', fontsize=14)
    ax.set_ylim([0, 1.05])
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3)
...
def train_epoch(model, dataloader, criterion, optimizer, device, evaluator, 
    ...
    for features, label in dataloader:
        ...
        # CRITICAL: Add these 3 lines for aggressive regularization
        features = F.dropout(features, p=0.3, training=True)  # Drop 30% of features
        ...
        # IMPORTANT: Ensure label is 1D tensor for CrossEntropyLoss
        if label.dim() == 0:
            label = label.unsqueeze(0)
        elif label.dim() > 1:
            label = label.squeeze()  # Remove extra dimensions
        ...
        # IMPORTANT: Ensure logits have correct shape [batch_size, n_classes]
        if logits.dim() == 1:
            logits = logits.unsqueeze(0)
        
        # Verify shapes before loss calculation
        assert logits.shape[0] == label.shape[0], f"Batch size mismatch: logits {logits.shape} vs labels {label.shape}"
        assert logits.dim() == 2, f"Logits should be 2D, got {logits.dim()}D"
        assert label.dim() == 1, f"Labels should be 1D, got {label.dim()}D"
...
def train_fold(model_config, train_loader, val_loader, device, 
    ...
    if USE_CLASS_WEIGHTS and class_weights is not None:
        ...
        # Just modify your existing criterion:
...
# This is the section in train_fold function that needs to be updated
# Replace the existing plotting call in your train_fold function with this:
