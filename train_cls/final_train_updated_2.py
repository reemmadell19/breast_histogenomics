...
def get_mil_hyperparameters(mil_architecture):
    """
    Get recommended hyperparameters for each MIL architecture from their papers.
    """
    ...
    mil_params = {
        ...
        'acmil': {
            ...
            'learning_rate': 1e-4,
            'weight_decay': 1e-4,
            'use_scheduler': True,  # Cosine scheduler
            'scheduler_type': 'cosine',
        }
    }
    
    if mil_architecture not in mil_params:
        raise ValueError(f"Unknown MIL architecture: {mil_architecture}")
    
    return mil_params[mil_architecture]
# =====================================================================
# CONFIGURATION - GLOBAL PARAMETERS
# =====================================================================

# Training hyperparameters (apply to all architectures)
NUM_EPOCHS = 100
BATCH_SIZE = 1
...
def create_model(input_dim, mil_architecture, device, n_classes=2):
    ...
    # Get hyperparameters for this MIL architecture
    mil_params = get_mil_hyperparameters(mil_architecture)
    ...
    if mil_architecture == "mean":
        model = model_class(
            input_dim=input_dim,
            hidden_dim=mil_params['hidden_dim'],
            n_classes=n_classes,
            dropout=mil_params['dropout']
        ).to(device)
        ...
        model = model_class(
            input_dim=input_dim,
            hidden_dim=mil_params['hidden_dim'],
            attention_hidden_dim=mil_params['attention_hidden_dim'],
            n_classes=n_classes,
            dropout=mil_params['dropout']
        ).to(device)
        ...
        model = model_class(
            ...
            n_classes=n_classes,
            dropout=mil_params['dropout'],
            gate=mil_params['gate'],
            instance_eval=mil_params['use_instance_learning'],
            k_sample=mil_params['k_sample'],
            instance_loss_fn=mil_params['instance_loss_fn']
        ).to(device)
        ...
        model = model_class(
            ...
            hidden_dim=mil_params['hidden_dim'],
            n_classes=n_classes,
            n_branches=mil_params['n_branches'],
            dropout=mil_params['dropout'],
            top_k=mil_params['top_k'],
            mask_ratio=mil_params['mask_ratio'],
            lambda_p=mil_params['lambda_p'],
            lambda_d=mil_params['lambda_d'],
            gate=mil_params['gate']
        ).to(device)
    
    return model, mil_params

def train_epoch(model, dataloader, criterion, optimizer, device, evaluator, mil_architecture, mil_params):
    ...
    for batch_features, batch_labels in dataloader:
        ...
        for i, features in enumerate(batch_features):
            ...
            if mil_architecture == "clam" and mil_params['use_instance_learning']:
        ...
        if mil_architecture == "acmil" and total_acmil_loss > 0:
            # ACMIL already computed its complete loss internally (bag + semantic + diversity)
            loss = total_acmil_loss / len(batch_features)
        elif mil_architecture == "clam" and mil_params['use_instance_learning'] and total_instance_loss > 0:
            ...
            loss = bag_loss + mil_params['instance_loss_weight'] * total_instance_loss
        else:
...
def train_fold(model_config, train_loader, val_loader, device, 
               fold_num, save_dir, mil_architecture):
    """Train a single fold and save best model"""
    set_seed(RANDOM_SEED + fold_num)
    
    n_classes = 2
    model, mil_params = create_model(model_config["input_dim"], mil_architecture, device, n_classes)
    
    # Use architecture-specific learning rate and weight decay
    learning_rate = mil_params['learning_rate']
    weight_decay = mil_params['weight_decay']
    
    # Setup loss
    criterion = FocalLoss(gamma=FOCAL_GAMMA, alpha=FOCAL_ALPHA)
    
    # Setup optimizer with architecture-specific parameters
    ...
    scheduler = None
    if mil_params.get('use_scheduler', False):
        if mil_params.get('scheduler_type') == "cosine":
    ...
    for epoch in pbar:
        train_loss = train_epoch(model, train_loader, criterion, optimizer, device, 
                                train_evaluator, mil_architecture, mil_params)
...
def run_cv_training(model_name, mil_architecture):
    ...
    # Save configuration - now includes architecture-specific scheduler info
    config_info = {
        ...
        'architecture_specific_params': {
            'hidden_dim': mil_params.get('hidden_dim'),
            'dropout': mil_params.get('dropout'),
            'learning_rate': mil_params.get('learning_rate'),
            'weight_decay': mil_params.get('weight_decay'),
            'use_scheduler': mil_params.get('use_scheduler', False),
            'scheduler_type': mil_params.get('scheduler_type', None) if mil_params.get('use_scheduler', False) else None,
        }
    }
    
    # Add architecture-specific parameters
    if mil_architecture == 'attention':
        config_info['architecture_specific_params']['attention_hidden_dim'] = mil_params.get('attention_hidden_dim')
    
    elif mil_architecture == 'clam':
        config_info['architecture_specific_params'].update({
            'attention_hidden_dim': mil_params.get('attention_hidden_dim'),
            'gate': mil_params.get('gate'),
            'use_instance_learning': mil_params.get('use_instance_learning'),
            'k_sample': mil_params.get('k_sample'),
            'instance_loss_fn': mil_params.get('instance_loss_fn'),
            'instance_loss_weight': mil_params.get('instance_loss_weight'),
        })
    
    elif mil_architecture == 'acmil':
        config_info['architecture_specific_params'].update({
            'n_branches': mil_params.get('n_branches'),
            'top_k': mil_params.get('top_k'),
            'mask_ratio': mil_params.get('mask_ratio'),
            'lambda_p': mil_params.get('lambda_p'),
            'lambda_d': mil_params.get('lambda_d'),
            'gate': mil_params.get('gate'),
        })
    ...
    for key, value in mil_params.items():
        print(f"  {key}: {value}")
