#!/usr/bin/env python3
"""
Feature extraction script for ECU dataset using ResNet50.
Processes JPEG-encoded 224x224 patches from tfrecord files.
ResNet50 outputs 2048-dimensional features vs ResNet18's 512.
"""
...
def extract_features_from_ecu_tfrecord(tfrecord_path, out_path):
    ...
    if all_features is not None and len(all_features) > 0:
        ...
        print(f"  ⚠️  No valid patches in {os.path.basename(tfrecord_path)}")
        return 0
...
def run_ecu_extraction(input_dir, output_dir, manifest_csv=None):
    ...
    print(f"\n" + "="*60)
    print(f"ECU RESNET50 FEATURE EXTRACTION")
    print(f"="*60)
    ...
    elapsed = time.time() - start_time
    print(f"\n" + "="*60)
    print(f"RESNET50 EXTRACTION SUMMARY")
    print(f"="*60)
    ...
    if processed_files > 0:
        ...
        print(f"⏱️  Average time per slide: {avg_time:.1f} seconds")
...
def verify_extraction(output_dir, sample_file=None):
    ...
    """
    print(f"\n" + "="*60)
    print(f"VERIFICATION - RESNET50 FEATURES")
    print(f"="*60)
