...
def plot_risk_distribution_pie(ecu_data, output_dir):
    ...
    plt.title('ECU Risk Distribution', fontsize=14, fontweight='bold')
...
def plot_race_distribution(ecu_data, output_dir):
    ...
    # Find race column
    race_col = None
    for col in ['race', 'Race', 'RACE', 'ethnicity', 'Ethnicity']:
        if col in ecu_data.columns:
            race_col = col
            break
    
    if race_col is None:
        print("No race/ethnicity column found")
        return
    ...
    if len(race_counts) > 6:
        top_races = race_counts.head(5)
        other_count = race_counts[5:].sum()
        if other_count > 0:
            top_races['Other'] = other_count
        race_counts = top_races
    ...
    plt.text(0, -1.2, f'Total patients with race data: {len(races)}', 
            ha='center', fontsize=11)
