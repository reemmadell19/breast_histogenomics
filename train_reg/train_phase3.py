...
# fixed script to avoid error in final validation
import os
import sys
...
# Configuration
CONFIG = {
    "feature_extractor": "resnet18",             # CHANGE THIS FOR EACH RUN
...
def setup_results_directory(feature_extractor: str, resume_dir: str = None) -> str:
    """Create or use existing results directory"""
    if resume_dir:
        results_dir = resume_dir
        print(f"Resuming with existing directory: {results_dir}")
    else:
...
def analyze_optimization_results(all_results, model_config, results_dir):
    ...
    # Extract best hyperparameters for saving - convert numpy types to Python native types
    best_hyperparams = {}
    for key in hyperparam_keys:
        value = best_result[key]
        if isinstance(value, np.integer):
            best_hyperparams[key] = int(value)
        elif isinstance(value, np.floating):
            best_hyperparams[key] = float(value)
        elif isinstance(value, np.bool_):
            best_hyperparams[key] = bool(value)
        else:
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Phase 3: CLAM Optimization')
    parser.add_argument('--feature_extractor', type=str, default=CONFIG["feature_extractor"],
                      help='Feature extractor to use (resnet18, h-optimus, virchow2, uni2-h)')
    parser.add_argument('--stage', type=str, default='all', 
                      choices=['all', 'grid_search', 'final_validation'],
                      help='Which stage to run')
    parser.add_argument('--resume_dir', type=str, default=None,
                      help='Directory with existing grid search results to resume from')
    parser.add_argument('--best_hyperparams_json', type=str, default=None,
                      help='Path to best_hyperparameters.json file for running final validation only')
    ...
    # Create or use existing results directory
    results_dir = setup_results_directory(feature_extractor, args.resume_dir)
    ...
    if args.stage == 'all' or args.stage == 'grid_search':
        # Save configuration
        ...
        # Save raw grid search results immediately
        with open(os.path.join(results_dir, "grid_search_raw_results.json"), 'w') as f:
            json.dump(all_results, f, indent=2, cls=NumpyEncoder)
        print(f"Grid search results saved to: {os.path.join(results_dir, 'grid_search_raw_results.json')}")
    ...
    elif args.stage == 'final_validation':
        # Load best hyperparameters from file
        if args.best_hyperparams_json:
            hyperparams_path = args.best_hyperparams_json
        else:
            hyperparams_path = os.path.join(results_dir, "best_hyperparameters.json")
        
        if not os.path.exists(hyperparams_path):
            print(f"Error: Cannot find hyperparameters file at {hyperparams_path}")
            print("Please provide --best_hyperparams_json or ensure grid search was completed")
            return
    ...
    if args.stage == 'all' or args.stage == 'final_validation':
    ...
    return final_summary if args.stage != 'grid_search' else None
