...
class WSIAttentionOverlay:
    ...
    def create_attention_overlay(self,
        ...
        # Create canvas for WSI reconstruction (pure white background)
    ...
    def visualize_overlay(self,
                         ...
                         alpha: float = 0.30,  # Much more transparent
                         colormap: str = 'RdBu_r',
        ...
        if create_multi_panel:
            ...
            # Detect tissue regions (non-white areas)
            # Better tissue detection - look for actual tissue
            gray_wsi = np.mean(wsi_canvas, axis=2)
            # Use a more aggressive threshold to only get real tissue
            tissue_mask = gray_wsi < 250  # Only very non-white areas
            
            # Clean up tissue mask
            from scipy.ndimage import binary_erosion, binary_dilation
            # Remove small noise
            tissue_mask = binary_dilation(binary_erosion(tissue_mask, iterations=2), iterations=2)
            
            # Apply attention only on tissue regions
            attention_masked = np.ma.masked_where(~tissue_mask, attention_smooth)
            
            # Use RdBu (not reversed) for blue (low) to red (high) like reference
            cmap = plt.get_cmap('RdBu_r')  # Blue to white to red
            # Alternative: 'Spectral_r' for blue to red through multiple colors
                ...
                try:
                    ...
                    if attention_weights is not None:
                        ...
                        wsi_canvas, attention_canvas = wsi_overlay_gen.create_attention_overlay(
                            ...
                            colormap='jet'  # Direct blue to red colormap
                        )
