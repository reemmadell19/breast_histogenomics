...
def save_model_mean_roc_data(all_fold_probs, labels, config, output_dir):
    """
    Save mean ROC curve data for this specific model configuration
    This data will be used to compare different feature extractors and MIL architectures
    """
    ...
    # Also calculate std of probabilities across folds for confidence intervals
    ...
    json_path = os.path.join(output_dir, f'{model_identifier}_mean_roc_data.json')
    with open(json_path, 'w') as f:
        json.dump(model_roc_data, f, indent=2)
    
    # 2. NumPy format for computational efficiency
    npz_path = os.path.join(output_dir, f'{model_identifier}_mean_roc_arrays.npz')
    np.savez(
        npz_path,
        fpr_mean=fpr_mean,
        tpr_mean=tpr_mean,
    ...
    csv_path = os.path.join(output_dir, f'{model_identifier}_mean_performance.csv')
    summary_df.to_csv(csv_path, index=False)
    
    print(f"\nMean ROC data saved for {model_identifier}:")
...
def test_individual_folds(model_folder, test_csv, output_dir=None, save_predictions=True):
    """Test each fold and save comprehensive results including mean ROC data"""
    ...
    if len(all_fold_probs) > 0 and saved_labels is not None:
        mean_roc_data = save_model_mean_roc_data(
            all_fold_probs,
            saved_labels,
            config,
            output_dir
    ...
    # Add mean ROC performance to summary
    if mean_roc_data:
        results_summary['mean_roc_performance'] = {
            'model_identifier': mean_roc_data['model_config']['identifier'],
            'mean_auc': mean_roc_data['mean_roc']['auc'],
            'auc_std': mean_roc_data['mean_roc']['auc_std'],
            'data_files': {
                'json': f"{mean_roc_data['model_config']['identifier']}_mean_roc_data.json",
                'numpy': f"{mean_roc_data['model_config']['identifier']}_mean_roc_arrays.npz",
                'csv': f"{mean_roc_data['model_config']['identifier']}_mean_performance.csv"
            }
        }
    ...
    if mean_roc_data:
        print(f"Mean ROC data saved for model comparison")
        print(f"  Model identifier: {mean_roc_data['model_config']['identifier']}")
        print(f"  Mean AUC: {mean_roc_data['mean_roc']['auc']:.3f} ± {mean_roc_data['mean_roc']['auc_std']:.3f}")
