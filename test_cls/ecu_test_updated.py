...
def run_interpretability_analysis(model, test_loader, test_csv, device, mil_architecture, 
    ...
    """Run interpretability analysis on ECU test set"""
    ...
    print(f"\nRunning interpretability analysis for ECU dataset...")
...
def test_individual_folds(model_folder, test_csv, output_dir=None, internal_test_results=None,
    ...
    """
    Test each fold individually on ECU dataset (external validation)
    
    Args:
        model_folder: Path to folder containing fold models
        test_csv: Path to ECU test CSV file
        output_dir: Output directory (optional)
        internal_test_results: Path to internal test results JSON for comparison (optional)
    ...
    # AFTER all folds are tested, run interpretability on the best one
    ...
    # If analyze_all_folds is True, run interpretability on all folds
...
######################
# python -B test_cls/ecu_test_updated.py --model-folder results_classification_cv_updated/h-optimus_clam_20250912_145351 --test-csv data/manifests/ecu_features_h-optimus.csv --run-interpretability    
